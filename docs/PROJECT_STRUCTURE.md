...
### <a name="realty-main"></a>`realty-main/`

Основной бэкенд-сервис платформы, построенный на **Django**. Этот сервис отвечает за всю основную бизнес-логику, связанную с недвижимостью, аналитикой, импортом данных и генерацией отчетов.

**Ключевые Django-приложения:**

-   **`realty/main/`**: Ядро сервиса.
    -   `models.py`: Содержит основные модели данных, такие как `Project`, `Building`, `MergedTransaction` (сделки по продажам), `MergedRentalTransaction` (сделки по аренде).
    -   `views.py`: Основные API endpoints для работы с данными.
    -   `tasks.py`: Асинхронные задачи (Celery) для обработки данных.
    -   `admin.py`: Настройка административной панели Django.

-   **`realty/pfimport/`**: Логика для импорта данных из Property Finder.
    -   `models.py`: Модели для хранения сырых данных (`PFJsonImport`, `PFListSale`, `PFListRent`).
    -   `management/commands/`: Django-команды для запуска процессов импорта.

-   **`realty/reports/`**: Генерация сложных аналитических отчетов.
    -   `models.py`: Модели для хранения сгенерированных отчетов (`BuildingReport`, `AreaReport`, `CityReport`).
    -   `views.py`: Endpoints для доступа к отчетам.

-   **`realty/building_reports/`**: Специализированные отчеты по конкретным зданиям.
    -   `models.py`: Модели для хранения "снимков" данных по зданиям (`BuildingReportSnapshot`).

**Важные файлы:**

-   **`manage.py`**: Стандартная утилита для управления Django-проектом.
-   **`requirements.txt`**: Список Python-зависимостей.
-   **`ansible/`**: Плейбуки для автоматизации развертывания.
-   **`project.json`**: Конфигурация для таск-раннера Nx, определяет команды `test`, `migrate` и т.д.
...
### <a name="pfimport-main"></a>`pfimport-main/`

Это второй Django-сервис, который отвечает за **импорт, парсинг и первичный анализ** данных о недвижимости, предположительно с Property Finder.

**Ключевые компоненты:**

-   **`parsing/`**: Содержит скрипты для скрейпинга (`run_scraper.sh`, `a_buy.py`). Это точка входа для получения сырых данных.
-   **`properties/`**: Основное Django-приложение.
    -   `models.py`: Модели для хранения данных об объектах (`Property`, `Building`) и их аналитики (`PropertyAnalytics`, `PropertyMetrics`).
    -   `management/commands/`: Набор Django-команд для управления данными: `import_properties`, `calculate_metrics`, `check_data` и т.д.
    -   `views.py`: API и HTML-представления для отображения импортированных данных.
-   **`property_analyzer/`**: Основной Django-проект, который объединяет приложение `properties`.
-   **`templates/`**: HTML-шаблоны для визуализации данных.

**Важные файлы:**

-   **`manage.py`**: Утилита для управления этим Django-проектом.
-   **`db.sqlite3`**: Локальная база данных SQLite, вероятно, используется для разработки.

---

### <a name="dxb-frontend-develop"></a>`DXB-frontend-develop/`

Фронтенд-приложение, отвечающее за весь пользовательский интерфейс платформы. Построено на **React** и **TypeScript**, с использованием **Vite** в качестве сборщика.

**Ключевые директории:**

-   **`src/`**: Основной исходный код приложения.
    -   **`api/`**: Логика для взаимодействия с бэкенд-сервисами. Содержит клиенты, `mutations` и `queries` (вероятно, для `react-query` или аналогичной библиотеки).
    -   **`components/`**: Переиспользуемые React-компоненты (UI-кит проекта), такие как `Chart`, `Header`, `PropertyCard`.
    -   **`pages/`**: Компоненты, представляющие собой целые страницы приложения (`Dashboard`, `Analytics`, `Auth`).
    -   **`hooks/`**: Кастомные React-хуки для переиспользования логики.
    -   **`services/`**: Сервисные классы для работы с внешними зависимостями (например, `memoryService`).
    -   **`locales/`**: Файлы для интернационализации (i18n).
    -   **`styles/`**: Глобальные стили и переменные (SCSS).

**Важные файлы:**

-   **`package.json`**: Список зависимостей и скриптов (`start`, `build`, `test`, `lint`).
-   **`vite.config.ts`**: Конфигурационный файл для сборщика Vite.
-   **`Dockerfile`**: Инструкции для сборки Docker-образа для продакшена.
-   **`.eslintrc.js`**: Конфигурация для линтера ESLint.
